...
public class MapleTrunkPlacer extends TrunkPlacer {
  ...
  public List<FoliageAttachment> placeTrunk(WorldGenLevel world, BiConsumer<BlockPos, BlockState> replacer, RandomSource random, int height, BlockPos startPos, TreeConfiguration config) {
  ...
  private FoliageAttachment generateBranch(WorldGenLevel world, BiConsumer<BlockPos, BlockState> replacer, RandomSource random, int height, BlockPos startPos, TreeConfiguration config,
  ...
  static {
    BRANCH_START_OFFSET_FROM_TOP_CODEC = UniformInt.MAP_CODEC.codec().validate((branchStartOffsetFromTop) -> {
      return branchStartOffsetFromTop.maxInclusive() - branchStartOffsetFromTop.minInclusive() < 1 ? DataResult.error(() -> {
    ...
    CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return trunkPlacerParts(instance).and(instance.group(IntProviders.codec(1, 5).fieldOf("branch_count").forGetter((trunkPlacer) -> {
        return trunkPlacer.branchCount;
      }), IntProviders.codec(1, 16).fieldOf("branch_horizontal_length").forGetter((trunkPlacer) -> {
        return trunkPlacer.branchHorizontalLength;
      }), IntProviders.validateCodec(-16, 0, BRANCH_START_OFFSET_FROM_TOP_CODEC).fieldOf("branch_start_offset_from_top").forGetter((trunkPlacer) -> {
        return trunkPlacer.branchStartOffsetFromTop;
      }), IntProviders.codec(-16, 16).fieldOf("branch_end_offset_from_top").forGetter((trunkPlacer) -> {
