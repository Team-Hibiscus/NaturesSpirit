      ...
      list.stream().filter((pos) -> pos.getY() < list.get(0).getY() + 6 && pos.getY() > list.get(0).getY()).forEach((pos) -> {
        ...
        if (random.nextFloat() <= this.small_probability) {
          for (Direction direction : Direction.Plane.HORIZONTAL.shuffledCopy(random)) {
            if (generator.isAir(pos.relative(direction))) {
              Direction direction2 = direction.getClockWise();
              generator.setBlock(pos.relative(direction), polypore_provider.getState(generator.level(), random, pos.relative(direction)).trySetValue(PolyporeBlock.FACING, direction));
              if (generator.isAir(pos.relative(direction2))) {
                generator.setBlock(pos.relative(direction2), polypore_provider.getState(generator.level(), random, pos.relative(direction2)).trySetValue(PolyporeBlock.FACING, direction2));
              }
              break;
            }
          }
    ...
    for (int j = 0; j <= radius; ++j) {
      for (int k = 0; k <= radius; ++k) {
        ...
        if (generator.isAir(blockPos)) {
          generator.setBlock(blockPos, block_provider.getState(generator.level(), random, blockPos).trySetValue(HugeMushroomBlock.DOWN, false));
        }
      }
    }
