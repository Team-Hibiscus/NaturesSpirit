...
public class MapleGroundTreeDecorator extends TreeDecorator {
  ...
  private void setColumn(Context generator, BlockPos origin) {
    for (int i = 2; i >= -3; --i) {
      ...
      if (generator.checkBlock(blockPos, (state) -> state.is(BlockTags.DIRT))) {
        generator.setBlock(blockPos, this.provider.getState(generator.level(), generator.random(), origin));
        break;
      }
  ...
  private void setColumn2(Context generator, BlockPos origin) {
    for (int i = 2; i >= -3; --i) {
      ...
      if (generator.checkBlock(blockPos, (state) -> state.is(BlockTags.DIRT))) {
        ...
        generator.setBlock(blockPos, this.provider2.getState(generator.level(), generator.random(), origin));
