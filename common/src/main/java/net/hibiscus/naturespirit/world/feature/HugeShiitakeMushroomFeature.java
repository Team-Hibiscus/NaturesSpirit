...
public class HugeShiitakeMushroomFeature extends AbstractHugeMushroomFeature {
  ...
  protected void makeCap(WorldGenLevel world, RandomSource random, BlockPos start, int y, MutableBlockPos mutable, HugeMushroomFeatureConfiguration config) {
        ...
        for (int m = -j; m <= j; ++m) {
          ...
          if (i >= y || (bl5 != bl6 && (i == y - 2 || i == y - 1))) {
            ...
            if (!world.getBlockState(mutable).isSolidRender()) {
              BlockState blockState = config.capProvider().getState(world, random, start);
              if (blockState.hasProperty(HugeMushroomBlock.WEST) && blockState.hasProperty(HugeMushroomBlock.EAST) && blockState.hasProperty(HugeMushroomBlock.NORTH) && blockState.hasProperty(
                  HugeMushroomBlock.SOUTH) && blockState.hasProperty(
  ...
  protected boolean isValidPosition(WorldGenLevel world, BlockPos pos, int height, MutableBlockPos mutablePos, HugeMushroomFeatureConfiguration config) {
    int i = pos.getY();
    if (i >= world.getMinY() + 1 && i + height + 1 <= world.getMaxY()) {
      if (!config.canPlaceOn().test(world, pos.below())) {
        return false;
      } else {
