...
public class NSSurfaceRules {
	...
	public static MaterialRules.MaterialRule makeRules() {
		...
		MaterialRules.MaterialRule steppeUndergroundRule = MaterialRules.condition(MaterialRules.biome(NSBiomes.SLEETED_SLOPES, NSBiomes.BLOOMING_HIGHLANDS, NSBiomes.SNOWCAPPED_RED_PEAKS, NSBiomes.SHRUBBY_HIGHLANDS, NSBiomes.WOODY_HIGHLANDS, NSBiomes.ARID_HIGHLANDS),
		...
		MaterialRules.MaterialRule tundraRule = MaterialRules.condition(belowWater,
			MaterialRules.condition(STONE_DEPTH_FLOOR_WITH_SURFACE_DEPTH,
				MaterialRules.condition(MaterialRules.biome(NSBiomes.SNOWY_FIR_FOREST, NSBiomes.TUNDRA, NSBiomes.BOREAL_TAIGA),
					MaterialRules.sequence(MaterialRules.condition(noiseCondition4, MaterialRules.condition(MaterialRules.biome(NSBiomes.TUNDRA, NSBiomes.BOREAL_TAIGA), stoneOrMoss)), MaterialRules.condition(MaterialRules.biome(
		...
		MaterialRules.MaterialRule tropicalBasinRule = MaterialRules.sequence(MaterialRules.condition(MaterialRules.STONE_DEPTH_FLOOR, MaterialRules.sequence(
				MaterialRules.condition(MaterialRules.biome(NSBiomes.MARSH, NSBiomes.TROPICAL_BASIN, NSBiomes.BAMBOO_WETLANDS), MaterialRules.condition(materialCondition6, MaterialRules.condition(MaterialRules.not(materialCondition4), MaterialRules.condition(MaterialRules.noiseThreshold(NoiseParametersKeys.SURFACE_SWAMP, 0.0), WATER))))
			))
		);
		MaterialRules.MaterialRule redwoodForestRule = MaterialRules.condition(MaterialRules.biome(NSBiomes.REDWOOD_FOREST),
			MaterialRules.sequence(
		...
		MaterialRules.MaterialRule alpineRule = MaterialRules.condition(MaterialRules.biome(NSBiomes.ALPINE_CLEARINGS, NSBiomes.ALPINE_HIGHLANDS, NSBiomes.CONIFEROUS_COVERT, NSBiomes.HEATHER_FIELDS),
			MaterialRules.sequence(
