		...
		for (int i = 0; i < height; ++i) {
			...
			if (i < height - 1) {
				...
				if (i > height - 9 && random.nextFloat() < .85 && i % 2 == 0) {
					...
					if (random.nextFloat() < .75)
						this.generateExtraBranch(world, replacer, random, config, nodes, startPos.offset(direction.rotateYClockwise(), 1), i, direction.rotateYClockwise().rotateYClockwise(), random.nextBetween(6, 9), 1);
