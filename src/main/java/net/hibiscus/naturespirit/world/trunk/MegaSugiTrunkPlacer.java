		...
		for (int i = 0; i < height; ++i) {
			...
			if (i < height - 1) {
				...
				if (i > 4 && i % 2 == 0) {
					...
					if (random.nextFloat() < .85)
						this.generateExtraBranch(world, replacer, random, height, config, nodes, startPos.offset(direction.rotateYClockwise(), 1), j, direction.rotateYClockwise().rotateYClockwise(), m, i);
