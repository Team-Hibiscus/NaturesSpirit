...
class NSBlockLootTableProvider extends FabricBlockLootTableProvider {
	...
	public final LootTable.Builder dropsWithSilkTouchOrShears2(ItemConvertible drop) {
		return LootTable.builder().pool(LootPool.builder().conditionally(this.createWithShearsOrSilkTouchCondition()).rolls(ConstantLootNumberProvider.create(1F)).with(ItemEntry.builder(drop)));
	}

	public LootTable.Builder noSaplingLeavesDrop(Block leaves) {
		RegistryWrapper.Impl<Enchantment> impl = this.registryLookup.getWrapperOrThrow(RegistryKeys.ENCHANTMENT);
		return this.dropsWithSilkTouchOrShears2(leaves)
			.pool(LootPool.builder().rolls(ConstantLootNumberProvider.create(1F)).conditionally(this.createWithoutShearsOrSilkTouchCondition()).with(((LeafEntry.Builder) this.applyExplosionDecay(leaves, ItemEntry.builder(Items.STICK).apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1F, 2F))))).conditionally(TableBonusLootCondition.builder(impl.getOrThrow(Enchantments.FORTUNE), LEAVES_STICK_DROP_CHANCE))));
	}
