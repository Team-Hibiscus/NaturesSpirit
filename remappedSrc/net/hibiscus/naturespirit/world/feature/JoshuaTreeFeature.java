...
public class JoshuaTreeFeature extends Feature <NoneFeatureConfiguration> {
   ...
   private static void generate2(LevelAccessor world, BlockPos pos, RandomSource random, BlockPos rootPos, int size, int layer) {
         ...
         for(int l = 0; l < k; ++l) {
            ...
            if (Math.abs(blockPos2.getX() - rootPos.getX()) < size && Math.abs(blockPos2.getZ() - rootPos.getZ()) < size && world.isEmptyBlock(blockPos2) && world.isEmptyBlock(blockPos2.below()) && isSurroundedByAir(world, blockPos2, direction.getOpposite())) {
               ...
               if(world.isEmptyBlock(blockPos2.above())) {
                  ...
                  world.setBlock(blockPos2.above(), HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
                                                                                 .setValue(LeavesBlock.DISTANCE, 1), 2);
                  world.setBlock(blockPos2,  ((JoshuaTrunkBlock) HibiscusWoods.JOSHUA[0]).withConnectionProperties(world, blockPos2), 2);
                  ...
                  if (random.nextBoolean() && world.isEmptyBlock(blockPos2.relative(direction, 1))) {
                     world.setBlock(blockPos2.relative(direction2, 1), HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
                                                                                                     .setValue(LeavesBlock.DISTANCE, 1), 2);
                     world.setBlock(blockPos2, ((JoshuaTrunkBlock) HibiscusWoods.JOSHUA[0]).withConnectionProperties(world, blockPos2), 2);
                  }
               } else
               if((world.isEmptyBlock(blockPos2.above(2)) && !world.isEmptyBlock(blockPos2.above()))) {
                  ...
                  world.setBlock(blockPos2.above(2), HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
                                                                                  .setValue(LeavesBlock.DISTANCE, 1), 2);
                  world.setBlock(blockPos2.above(1),  ((JoshuaTrunkBlock) HibiscusWoods.JOSHUA[0]).withConnectionProperties(world, blockPos2.above()), 2);

                  Direction direction2 = Direction.Plane.HORIZONTAL.getRandomDirection(random);
                  if (random.nextBoolean() && world.isEmptyBlock(blockPos2.above().relative(direction, 1))) {
                     world.setBlock(blockPos2.above().relative(direction2, 1), HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
                                                                                                          .setValue(LeavesBlock.DISTANCE, 1), 2);
                     world.setBlock(blockPos2.above(), ((JoshuaTrunkBlock) HibiscusWoods.JOSHUA[0]).withConnectionProperties(world, blockPos2.above()), 2);
                  }
               }
            }
         }
      ...
      if(bl) {
         world.setBlock(pos.above(i), HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
                                                                   .setValue(LeavesBlock.DISTANCE, 1), 2);
         world.setBlock(pos.above(i - 1), ((JoshuaTrunkBlock) HibiscusWoods.JOSHUA[0]).withConnectionProperties(world, pos.above(i - 1)), 2);
         Direction direction = Direction.Plane.HORIZONTAL.getRandomDirection(random);
         if (random.nextBoolean() && world.isEmptyBlock(pos.above(i-1).relative(direction, 1))) {
            world.setBlock(pos.above(i-1).relative(direction, 1),
                    HibiscusWoods.JOSHUA_LEAVES.defaultBlockState()
