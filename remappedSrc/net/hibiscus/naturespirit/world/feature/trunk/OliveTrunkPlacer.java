...
public class OliveTrunkPlacer extends TrunkPlacer {
   ...
   protected TrunkPlacerType <?> type() {
      return HibiscusWorldGen.OLIVE_TRUNK_PLACER;
   }
   ...
   public List <FoliageAttachment> placeTrunk(LevelSimulatedReader world, BiConsumer <BlockPos, BlockState> replacer, RandomSource random, int height, BlockPos startPos, TreeConfiguration config) {
      ...
      if(bl3 && bl) {
         for(int i = 0; i < height - randomInt; ++i) {
            ...
            BlockPos pos2 = mutablePos1.relative(trunkOffset.getOpposite()).relative(randomInt2 < 0 ? Direction.UP : Direction.DOWN, randomInt2);
            BlockPos pos3 = pos2.relative(trunkOffset2.getOpposite());
            BlockPos blockPos = i >= 2 ? bl2 && trunkOffset.getOpposite() != trunkOffset2.getOpposite() ? pos3 : pos2 : mutablePos1.relative(randomInt2 < 0 ? Direction.UP : Direction.DOWN, randomInt2);

            if(i == 2) {
               this.placeLog(world, replacer, random, mutablePos1.relative(randomInt2 < 0 ? Direction.UP : Direction.DOWN, randomInt2), config);
            }
            if(this.placeLog(world, replacer, random, blockPos, config) && i < height - 1 && random.nextFloat() < this.placeBranchPerLogProbability) {
               int k = this.extraBranchLength.sample(random);
               int l = Math.max(0, k - this.extraBranchLength.sample(random) - 1);
