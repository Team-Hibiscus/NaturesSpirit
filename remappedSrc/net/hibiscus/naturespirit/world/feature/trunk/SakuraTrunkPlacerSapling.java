...
public class SakuraTrunkPlacerSapling extends TrunkPlacer {
    public static final Codec <SakuraTrunkPlacerSapling> CODEC = RecordCodecBuilder.create((instance) -> {
        return fillTrunkPlacerFields(instance).apply(instance, SakuraTrunkPlacerSapling::new);
    });
    ...
    public List <FoliagePlacer.TreeNode> generate(TestableWorld levelSimulatedReader, BiConsumer <BlockPos, BlockState> biConsumer, Random randomSource, int i, BlockPos blockPos, TreeFeatureConfig treeConfiguration) {
        ...
        double d = 1.0D;
        int m = Math.min(2, MathHelper.floor(1.482D + Math.pow(1.0D * (double) k / 11.0D, 2.0D)));
        int n = blockPos.getY() + l;
        int o = k - 6;
        List <SakuraTrunkPlacerSapling.FoliageCoords> list = Lists.newArrayList();
        list.add(new SakuraTrunkPlacerSapling.FoliageCoords(blockPos.up(o), n));
    ...
    private void makeBranches(TestableWorld levelSimulatedReader, BiConsumer <BlockPos, BlockState> biConsumer, Random randomSource, int i, BlockPos blockPos, List <SakuraTrunkPlacerSapling.FoliageCoords> list, TreeFeatureConfig treeConfiguration) {
