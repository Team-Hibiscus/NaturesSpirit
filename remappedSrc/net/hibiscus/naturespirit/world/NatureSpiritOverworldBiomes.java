...
public class NatureSpiritOverworldBiomes {
   ...
   private static Biome biome(boolean precipitation, float temperature, float downfall, int waterColor, int waterFogColor, MobSpawnSettings.Builder spawnBuilder, BiomeGenerationSettings.Builder biomeBuilder, @Nullable Music music) {
      return (new Biome.BiomeBuilder()).hasPrecipitation(precipitation).temperature(temperature).downfall(downfall).specialEffects((new BiomeSpecialEffects.Builder())
   ...
   public static Biome erodedRiver(HolderGetter <PlacedFeature> featureLookup, HolderGetter <ConfiguredWorldCarver <?>> carverLookup) {
      MobSpawnSettings.Builder builder = (new MobSpawnSettings.Builder()).addSpawn(MobCategory.WATER_CREATURE, new MobSpawnSettings.SpawnerData(EntityType.SQUID, 2, 1, 4)).addSpawn(MobCategory.WATER_AMBIENT,
      ...
      BiomeGenerationSettings.Builder lookupBackedBuilder = new BiomeGenerationSettings.Builder(featureLookup, carverLookup).addFeature(GenerationStep.Decoration.FLUID_SPRINGS, RIVER_WATER);
   ...
   public static Biome redwoodForest(HolderGetter <PlacedFeature> holderGetter, HolderGetter <ConfiguredWorldCarver <?>> holderGetter2) {
      ...
      BiomeDefaultFeatures.addDefaultExtraVegetation(biomeBuilder);
      return biomeWithGrassColor(true, 0.4F, 0.6F, 11451757, spawnBuilder, biomeBuilder, Musics.createGameMusic(SoundEvents.MUSIC_BIOME_FOREST));
   }
