...
public class FirFoliagePlacer extends FoliagePlacer {
   ...
   protected FoliagePlacerType <?> type() {
      return HibiscusForgeWorldGen.FIR_FOLIAGE_PLACER_TYPE.get();
   }
         ...
         if(l >= offset - 2) {
            ...
            if(random.nextBoolean()) {
               ...
               tryPlaceLeaf(world,
                       ...
                       random,
                       config,
                       mutable.relative(m.getOpposite(), 1).relative(n.getOpposite(), 1)
               );
            }
            if(random.nextBoolean()) {
               Direction m = random.nextBoolean() ? Direction.NORTH : Direction.SOUTH;
            ...
            tryPlaceLeaf(world,
                    ...
                    random,
                    config,
                    mutable.relative(Direction.NORTH, 2).relative(Direction.EAST, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.NORTH, 2).relative(Direction.WEST, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.EAST, 2).relative(Direction.NORTH, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.EAST, 2).relative(Direction.SOUTH, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.SOUTH, 2).relative(Direction.EAST, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.SOUTH, 2).relative(Direction.WEST, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.WEST, 2).relative(Direction.NORTH, 1)
            );
            tryPlaceLeaf(world,
                    placer,
                    random,
                    config,
                    mutable.relative(Direction.WEST, 2).relative(Direction.SOUTH, 1)
            );
            this.placeLeavesRow(world, placer, random, config, blockPos, 1, l, treeNode.doubleTrunk());
         }
         else if(l == offset - 11) {
